...
def parse_ticketmaster_email(subject: str, html: str) -> Optional[dict]:
    ...
    if subject:
        ...
        if m:
            order_id = m.group(1)
    if not order_id:
        m = re.search(r"r[ée]f[ée]rence\s*n[°o]?\s*(\d+)", text, re.IGNORECASE)
        if m:
            order_id = m.group(1)
    if not order_id:
        return None
    ...
    # --- Price ---
    price = "0"
    m = re.search(r"Total de la commande\s*:?\s*(\d+)", text, re.IGNORECASE)
    if m:
        price = m.group(1)
    if price == "0":
        m = re.search(r"Montant\s+total\s*:?\s*(\d+)", text, re.IGNORECASE)
        if m:
            price = m.group(1)
